...
Este estudo foi conduzido em conformidade com as diretrizes PRISMA~2020 para revisões sistemáticas e meta-análises \citep{Page2021}. 

A Fig.~\ref{fig:prisma-flow} apresenta o diagrama de fluxo PRISMA~2020, que sintetiza o processo de identificação, triagem, deduplicação e seleção dos 48 estudos elegíveis a partir das duas bases consultadas.
...
As buscas foram complementadas por rastreamento de referências dos estudos incluídos (\textit{backward snowballing}) e citações dos estudos incluídos (\textit{forward snowballing} via Google Scholar), para minimizar omissão de estudos relevantes não indexados nas bases primárias. Todos os registros foram exportados em formato BibTeX, unificados em base de dados consolidada e submetidos a remoção automática de duplicatas mediante o pacote \textit{revtools} do R \citep{Westgate2019}.
...
\noindent Quando apenas intervalos de confiança foram disponíveis, médias e desvios-padrão foram estimados conforme \citet{Betancur2022}. Dados apresentados exclusivamente em figuras foram digitalizados mediante WebPlotDigitizer v4.3 \citep{Rohatgi2020}. Quando estudos reportaram resultados para múltiplas comunidades, práticas ou períodos, os dados foram extraídos separadamente para cada condição, preservando a resolução e evitando que efeitos de tratamento fossem obscurecidos por agregação.
...
\noindent A Equação~\ref{eq:z-conv} converte o p-value bilateral no quantil $z$ da distribuição normal padrão, e a Equação~\ref{eq:d-conv} traduz $z$ na diferença de médias padronizada $d$ \citep{Borenstein2009}. A Equação~\ref{eq:lnor-conv} transforma $d$ em log-odds ratio pela conversão de \citet{Hasselblad1995}, e a Equação~\ref{eq:lnrr-conv} converte o $lnOR$ resultante em $lnRR$ pela transformação de \citet{Zhang1998} com prevalência basal $p_0 = 0{,}5$ como prior não-informativo. Quando o coeficiente de variação ($CV$) mediano dos estudos Tier~1 na mesma dimensão estava disponível, a conversão alternativa
%
\begin{equation}
...
\noindent proposta por \citet{Lajeunesse2011} foi empregada por sua maior precisão.
...
A composição do corpus (94\% dos registros sem médias e desvios-padrão completos) tornou impraticável a imputação múltipla por equações encadeadas (MICE), cuja validade requer proporção substancial de dados observados para sustentar as distribuições condicionais \citep{vanBuuren2011}. Em substituição, adotou-se o protocolo de integração multi-tier descrito na Seção~\ref{sec:multitier}, que converte evidência qualitativa e semiquantitativa em $lnRR$ sintéticos com propagação explícita de incerteza por tier. Essa abordagem preservou todos os 47~estudos elegíveis (222~registros, $k = 37$ por dimensão), evitando o viés de exclusão que resultaria de restringir a meta-análise aos cinco estudos com dados quantitativos completos. Análises de sensibilidade compararam estimativas do modelo completo ($k = 37$) com o subconjunto restrito a registros Tier~1 ($k_{T1} = 3\text{--}5$, conforme dimensão) para avaliar a influência da composição de tiers nos resultados.

A base de dados foi estruturada hierarquicamente em três níveis para controlar dependências estatísticas decorrentes de múltiplas comparações extraídas de um mesmo estudo \citep{Pustejovsky2022}. O Nível~1 (observação) representou cada comparação pareada entre intervenção e controle, vinculada a um indicador proxy específico, com $lnRR$ e variância amostral calculados. O Nível~2 (estudo) agrupou observações provenientes da mesma publicação, com efeitos aleatórios capturando condições compartilhadas (contexto geográfico, equipe de pesquisa, protocolo de coleta). O Nível~3 (meta-análise) incorporou efeitos fixos (moderadores) e a variância residual entre estudos ($\tau^2$), estimados por REML. 

Essa estrutura evita a inflação artificial da precisão que resultaria de tratar observações dependentes como independentes \citep{Cheung2014}. A matriz de variância-covariância dos efeitos intra-estudo ($\mathbf{V}_j$) foi construída assumindo correlação constante $\rho$ entre pares de tamanhos de efeito provenientes do mesmo estudo, conforme o modelo de efeitos correlados de \citet{Pustejovsky2022}, com valor de referência $\rho = 0{,}8$. Análises de sensibilidade variaram $\rho$ no intervalo $[0{,}5;\; 1{,}0]$ para avaliar a robustez das estimativas globais e dos intervalos de confiança à especificação da correlação intra-estudo. A estimação robusta de variância (\textit{robust variance estimation}, RVE) com correção de pequena amostra (CR2) foi aplicada via pacote \textit{clubSandwich} (versão $\geq$ 0.5.5) para produzir erros-padrão válidos independentemente da especificação exata de $\rho$ \citep{Pustejovsky2022}.
...
O logaritmo da razão de resposta ($lnRR$) foi adotado como métrica de tamanho de efeito por sua interpretabilidade proporcional e menor sensibilidade a \textit{outliers} comparado à diferença de médias padronizada \citep{Hedges1999}, sendo calculado conforme Equação~\ref{eq:lnrr}.
...
\noindent Valores de $lnRR > 0$ indicam melhoria do indicador sob intervenção de salvaguarda (aumento de diversidade, transmissão, documentação), enquanto $lnRR < 0$ indicam deterioração. Para facilitar a interpretação, os $lnRR$ foram retrotransformados em variação percentual pela expressão $(e^{lnRR} - 1) \times 100$. A variância amostral associada a cada $lnRR$ foi estimada conforme Equação~\ref{eq:vi} \citep{Hedges1999}.
...
O intervalo de predição complementou o IC~95\% para indicar a faixa esperada de efeitos em estudos futuros \citep{IntHout2016}. Para dimensões com $k < 15$, aplicou-se adicionalmente a correção de Hartung-Knapp-Sidik-Jonkman (HKSJ), que emprega a distribuição $t$ com $k-1$ graus de liberdade, produzindo cobertura mais conservadora \citep{Higgins2019}. Estimativas de $\tau^2$ foram comparadas entre os métodos DerSimonian-Laird e REML para avaliar robustez à escolha do estimador \citep{Borenstein2009}.

A identificação de estudos atípicos ou influentes foi conduzida por resíduos estudentizados com correção de Bonferroni ($\alpha = 0{,}05$) e pela distância de Cook, com limiar $D_i > 4/k$ \citep{Viechtbauer2010}. Análises de sensibilidade \textit{leave-one-out} foram conduzidas para todas as dimensões, e a técnica \textit{leave-one-study-out} (excluindo todas as observações de um estudo de cada vez) foi aplicada para avaliar a dependência dos resultados a publicações individuais.

O viés de publicação foi avaliado por inspeção visual de funnel plots (dispersão de $lnRR$ vs.\ erro-padrão), complementada pelo teste de Begg e Mazumdar \citep{Begg1994} e pelo teste de Egger \citep{Sterne2005}. Para dimensões com $k < 10$, os testes foram reportados mas interpretados com cautela devido ao baixo poder estatístico, conforme \citet{Sterne2011}. O método \textit{trim-and-fill} de Duval e Tweedie foi aplicado como análise suplementar para estimar estudos potencialmente faltantes e o efeito ajustado \citep{Duval2000}.
...
Para as dimensões $V_3$ e $V_5$, cujas estimativas a priori eram previstas como exploratórias, a integração multi-tier elevou $k$ ao limiar confirmatório, embora a base de evidência direta (Tier~1) permaneça restrita e as estimativas devam ser interpretadas com cautela. Os outputs consolidados orientam investigações subsequentes, incluindo a construção futura de um índice composto de salvaguarda biocultural.

\subsection{Análise de lacunas de evidência}
\label{sec:gaps}

Para cada dimensão funcional ($V_1$--$V_6$), foi conduzida uma análise sistemática de lacunas de evidência, contabilizando o número de variáveis proxy listadas na Tabela~\ref{tab:proxies} para as quais nenhum estudo elegível reportou dados quantitativos suficientes para cálculo de $lnRR$. Essa contagem produz um vetor de ``evidência ausente'' por dimensão, que identifica as variáveis não cobertas pela literatura empírica existente e que, portanto, demandam elicitação em investigações complementares futuras. Adicionalmente, para cada variável proxy com evidência disponível, foi registrado se os dados provêm exclusivamente de contextos geográficos sem congruência com o Semiárido NE~II (moderador binário = 0), sinalizando variáveis cuja validade de transferência é condicionada e que requerem validação territorial complementar. O mapeamento entre indicadores extraídos da meta-análise e dimensões funcionais de vulnerabilidade é consolidado na Tabela~S3.
