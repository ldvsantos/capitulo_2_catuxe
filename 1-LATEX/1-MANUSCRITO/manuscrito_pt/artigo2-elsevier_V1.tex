...
O framework postula que a vulnerabilidade agregada resulta da interação de oito dimensões funcionais ($V_1$--$V_8$), quantificáveis por indicadores proxy da literatura empírica (Tabela~\ref{tab:proxies}). $V_6$ (Vitalidade Linguística) é mensurável pela escala EGIDS \citep{LewisSimons2010} e pelo índice VITEK \citep{ZentMaffi2009}, $V_7$ (Integração ao Mercado) pela proporção de produção destinada à venda e pela taxa de substituição de variedades crioulas \citep{Godoy2005,ReyesGarcia2013}, e $V_8$ (Exposição Climática) pelo SPI e pela frequência de eventos extremos \citep{Savo2016}.
...
Os autores agradecem ao Programa de Pós-Graduação em Ciências da Propriedade Intelectual (PPGPI/UFS) pelo apoio institucional.
